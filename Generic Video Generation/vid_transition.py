...
def create_transition_animation(args_dict):
    all_animation_names = ", ".join([animation_enum.name for animation_enum in Animations])
    # print(args_dict)

    # Set default values if not present in args_dict
    args = {
        'input': args_dict.get('input', INPUT_VIDEOS),
        'num_frames': args_dict.get('num_frames', NUM_FRAMES),
        'animation': args_dict.get('animation', ANIMATION),
        'output': args_dict.get('output', OUTPUT),
        'max_rotation': args_dict.get('max_rotation', MAX_ROTATION),
        'max_distortion': args_dict.get('max_distortion', MAX_DISTORTION),
        'max_blur': args_dict.get('max_blur', MAX_BLUR),
        'max_brightness': args_dict.get('max_brightness', MAX_BRIGHTNESS),
        'max_zoom': args_dict.get('max_zoom', MAX_ZOOM),
        'debug': args_dict.get('debug', DEBUG),
        'art': args_dict.get('art', ART),
        'remove': args_dict.get('remove', REMOVE_ORIGINAL),
        'merge': args_dict.get('merge', MERGE_PHASES)
    }

    print(args)
    # Check if the animation argument is set to "help"
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        actions_determinator = AnimationActions(args['max_zoom'], args['max_brightness'], args['max_rotation'],
                                                args['max_blur'], args['max_distortion'], args['num_frames'])
...
#     create_transition_animation(args)
...
def addVideoTransition(videos , animation , output):
    # print(videos)
...
# if __name__ == "__main__":
#     tempFunction(['Scene1.mp4' , 'Scene2.mp4'] , 'translation')